...
function initBattery(){
    ...
    navigator.getBattery().then((batt)=>{
        updateBattery = () =>{
            ...
            else if(level<=20 &! batt.charging){ // We validate if the battery is charging
                batteryStatus.innerHTML = 'Low battery <i class="ri-plug-line animated-red"></i>'
            }
            else if(batt.changing){ //Validate if the battery is charging
            ...
            if(level <= 20){
                ...
                batteryLiquid.classList.remove('gradient-color-orange','gradient-color-yellow','gradient-color-red');
            }
        }
        updateBattery();
